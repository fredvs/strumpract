...
// Set the RTL Events Global (will pause/start/replay all the players synchro with same rtl event)) 
 ...
 {$endif}
      ...
      function  AddIntoMemoryBuffer(outmemory: PDArFloat; SampleRate: LongInt;  SampleFormat:
                                    LongInt;
...
{$IF DEFINED(synthesizer)}
      function AddFromSynth(Frequency: float; VolumeL: float; VolumeR: float; Duration: cint32;
                            OutputIndex: cint32;
                            SampleFormat: cint32 ; SampleRate: cint32 ; FramesCount : cint32):
                                                                                              cint32
      ;
...
// OutputIndex : Output index of used output// -1: all output, -2: no output, other cint32 refer to a existing OutputIndex  (if multi-output then OutName = name of each output separeted by ';')
      ...
      procedure InputSetSynth(InputIndex: cint32; Frequency: float; VolumeL: float; VolumeR: float;
                              Duration: cint32; Enable : boolean);
...
// TypeFilter: Type of filter : default = -1 = fBandSelect (fBandAll = 0, fBandSelect = 1, fBandReject = 2
      ...
      procedure InputSetFilter(InputIndex: cint32; FilterIndex: cint32;
                               ...
                               TypeFilter: cint32; AlsoBuf: boolean; Enable: boolean; LoopProc:
                               TProc );
      // InputIndex : InputIndex of a existing Input
        ...
        begin
          ...
          case TypeFilter of 
            ...
            5: //  DSPFFTHighPass
               begin
                 StreamIn[InputIndex].DSP[FilterIndex].fftdata.C := 
                                                                    Tan(Pi * LowFrequency / StreamIn
                                                                    [InputIndex].Data.SampleRate);
...
procedure Tuos_Player.OutputSetFilter(OutputIndex: cint32; FilterIndex: cint32;
                                      LowFrequency: cint32; HighFrequency: cint32; Gain: cfloat;
                                      TypeFilter: cint32; AlsoBuf: boolean; Enable: boolean;
                                      LoopProc: TProc);
// OutputIndex : OutputIndex of a existing Output
// FilterIndex : DSPOutIndex of existing DSPOut
// LowFrequency : Lowest frequency of filter
        ...
        4: //  DSPFFTLowPass
           begin
             ...
             StreamOut[OutputIndex].DSP[FilterIndex].fftdata.a3[0] := 
                                                                      ...
                                                                      FilterIndex].fftdata.C);
             StreamOut[OutputIndex].DSP[FilterIndex].fftdata.a3[1] := 
                                                                      2 * StreamOut[OutputIndex].DSP
                                                                      [FilterIndex].fftdata.a3[0];
             StreamOut[OutputIndex].DSP[FilterIndex].fftdata.a3[2] := 
                                                                      StreamOut[OutputIndex].DSP[
                                                                      FilterIndex].fftdata.a3[0];
             StreamOut[OutputIndex].DSP[FilterIndex].fftdata.b2[0] := 
                                                                      2 * (1 - StreamOut[OutputIndex
                                                                      ].DSP[FilterIndex].fftdata.C *
                                                                      StreamOut[OutputIndex].DSP[
                                                                      FilterIndex].fftdata.C) *
                                                                      StreamOut[OutputIndex].DSP[
...
begin
  ...
  StreamIn[InputIndex].DSP[result].fftdata.FNoise := 
                                                     ...
                                                     data.Channels,StreamIn[InputIndex].data.
                                                     SampleRate);
...
begin
  ...
  StreamOut[OutputIndex].DSP[result].fftdata.FNoise := 
                                                       ...
                                                       ].data.Channels,StreamOut[OutputIndex].data.
                                                       SampleRate);
...
function Tuos_Player.InputAddFilter(InputIndex: cint32; LowFrequency: cint32;
                                    HighFrequency: cint32; Gain: cfloat; TypeFilter: cint32; AlsoBuf
                                    : boolean;
...
{$IF DEFINED(synthesizer)}
function Tuos_Player.AddFromSynth(Frequency: float; VolumeL: float; VolumeR: float; duration :
                                  cint32; OutputIndex: cint32;
                                  SampleFormat: cint32 ; SampleRate: cint32 ; FramesCount : cint32):
                                                                                              cint32
;
// Add a input from Synthesizer with custom parameters
...
procedure Tuos_Player.InputSetSynth(InputIndex: cint32; Frequency: float; VolumeL: float; VolumeR:
                                    float;  Duration: cint32; Enable : boolean);
// Frequency : in Hertz (-1 = do not change)
// VolumeL :  from 0 to 1 (-1 = do not change)
// VolumeR :  from 0 to 1 (-1 = do not change)
// Duration : in msec (-1 = do not change)
...
begin
  ...
  if Duration <> -1 then  StreamIn[InputIndex].Data.dursine := trunc( StreamIn[InputIndex].Data.
...
function  Tuos_Player.AddIntoMemoryBuffer(outmemory: PDArFloat; SampleRate: LongInt;  SampleFormat:
                                          LongInt;
                                          Channels: LongInt; FramesCount: LongInt): LongInt;
// Add a Output into TMemoryStream
// outmemory : pointer of buffer to use to store memory.
...
begin
 ...
 {$endif}
  ...
  {$IF DEFINED(portaudio)}
  err := Pa_OpenStream(@StreamOut[x].Data.HandleSt, Nil, @StreamOut[x].PAParam, CDouble(StreamOut[x]
...
begin
  ...
  if assigned(MemoryStream) then
    begin
      ...
      {$IF DEFINED(mpg123)}
  // mpg123
  if ((StreamIn[x].Data.LibOpen = -1)) then if (((TypeAudio=-1) and (err=-1) ) or ((TypeAudio = 1)
                                               and (StreamIn[x].Data.LibOpen = -1) and (
...
{// problems with mpg123 library
        ...
        // to do : add id2v2
            if (mpid3v1 <> nil) and  (mpid3v1^ <> nil)  then begin
               ...
               StreamIn[x].Data.genre := refmpid3v1.genre;
                                                    ...
                                                    StreamIn[x].Data.LibOpen := 1;
                                                    StreamIn[x].Data.Length := mpg123_length(
                                                                               StreamIn[x].Data.
                                                                               HandleSt);
      ...
      {$IF DEFINED(opus)}
  if (((TypeAudio=-1) and (err=-1) ) or (TypeAudio = 2) and (StreamIn[x].Data.LibOpen = -1) and (
     uosLoadResult.OPloadERROR = 0)) then
    begin
      Err := -1;
...
begin
  ...
  StreamIn[x].Data.posdursine := StreamIn[x].Data.posdursine + (StreamIn[x].Data.WantFrames Div
                                 StreamIn[x].Data.Channels);
  ...
  if (StreamIn[x].Data.posdursine <= StreamIn[x].Data.dursine) or (StreamIn[x].Data.dursine = 0)
    ...
    begin
      ...
      while x2 < (StreamIn[x].Data.WantFrames * StreamIn[x].Data.Channels) - (StreamIn[x].Data.
            Channels -1)  do
        ...
        begin
          ...
          if StreamIn[x].Data.Channels = 2 then
            begin
              sf1 := StreamIn[x].Data.VLeft * CFloat((Sin( ( CFloat((x2 Div 2)+ StreamIn[x].Data.
                     posLsine)/CFloat( StreamIn[x].Data.lensine) ) * Pi * 2 )));
              sf2 := StreamIn[x].Data.VRight * CFloat((Sin( ( CFloat((x2 Div 2) + StreamIn[x].Data.
                     posRsine)/CFloat( StreamIn[x].Data.lensine) ) * Pi * 2 )));
              case StreamIn[x].Data.SampleFormat of 
                2: // int16
                   begin
          ...
          if StreamIn[x].Data.Channels = 1 then
            begin
              sf1 := StreamIn[x].Data.VLeft * CFloat((Sin( ( CFloat(x2+ StreamIn[x].Data.posLsine)/
