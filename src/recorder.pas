...
begin
  ...
  edvol.frame.buttonsize   := roundmath(22 * ratio);
  edvolr.frame.buttonsize  := roundmath(22 * ratio);
  ...
  with tgroupbox1 do
    for i1 := 0 to childrencount - 1 do
      for i2 := 0 to length(boundchildre) - 1 do
        ...
        begin
          children[i1].left   := roundmath(boundchildre[i2].left * ratio);
          children[i1].top    := roundmath(boundchildre[i2].top * ratio);
          children[i1].Width  := roundmath(boundchildre[i2].Width * ratio);
          children[i1].Height := roundmath(boundchildre[i2].Height * ratio);
        end;
...
begin
  ...
  poswavrec2.y := ((rectrecform.cy div 2) - 1) - roundmath((lv) * ((rectrecform.cy div 2) - 3));
  waveforec.sliderimage.bitmap.Canvas.drawline(poswavrec, poswavrec2, $AC99D6);

  poswavrec.y := (waveforec.trackbar1.Height div 2);

  poswavrec2.y := poswavrec.y + (roundmath((rv) * ((waveforec.trackbar1.Height div 2) - 3)));
